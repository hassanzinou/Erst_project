namespace SQLExecuter
{
    ...
    // Der Advanced Encryption Standard (AES) ist ein symmetrisches Kryptosystem, das als Nachfolger für DES bzw. 3DES im 
    // Oktober 2000 vom National Institute of Standards and Technology (NIST) als Standard bekannt gegeben wurde. Nach 
    // seinen Entwicklern Joan Daemen und Vincent Rijmen wird er auch Rijndael-Algorithmus genannt 
    // (gesprochen wie dt. „Reyndahl“).
    // Der Rijndael-Algorithmus besitzt eine variable Blockgröße von 128, 192 oder 256 Bit und eine variable 
    // Schlüssellänge von 128, 192 oder 256 Bit. Rijndael bietet ein sehr hohes Maß an Sicherheit. Das Verfahren wurde 
    // eingehenden kryptoanalytischen Prüfungen unterzogen. AES schränkt die Blocklänge auf 128 Bit ein, während die Wahl 
    // der Schlüssellänge von 128, 192 oder 256 Bits unverändert übernommen worden ist. Anhand der Schlüssellänge wird 
    // zwischen den drei AES-Varianten AES-128, AES-192 und AES-256 unterschieden.
    // Der Algorithmus ist frei verfügbar und darf ohne Lizenzgebühren eingesetzt sowie in Software bzw. Hardware 
    // implementiert werden. AES ist in den USA für staatliche Dokumente mit höchster Geheimhaltungsstufe zugelassen.
    
    //
    //    This sample code is provided "AS IS" with no warranties, and confers no rights.
    //
    //    ATTENTION: This sample is designed to be more of a tutorial rather than something
    //                you can copy and paste in the production code!
    //                See the topic on "making it industry strength" in the blog.
    //
    ...
    public class EncDec
    {
        #region MasterPasswort
        private const string masterPassword = "ICCIn1Encod3r4S1ch3rh31t";
        #endregion
        ...
        {
            return Encrypt(clearText, masterPassword);
        }
        ...
        {
            return Encrypt(clearData, masterPassword);
        }
        ...
        {
            return Decrypt(cipherData, masterPassword);
        }
